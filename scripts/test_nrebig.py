...
def test_tess_targets():
    print("\n" + "=" * 75)
    print("TESTING DEEP TRANSITING PLANETS FROM D:/nrebig (TESS DATASETS)")
    print("=" * 75)

    targets_data = json.load(open('D:/nrebig/tess/targets.json', encoding='utf-8'))
    tess_dirs = [d for d in os.listdir('D:/nrebig/tess/lightcurves') if os.path.isdir(os.path.join('D:/nrebig/tess/lightcurves', d))]
    ...
    print(f"Found {len(planets_to_test)} downloaded TESS target folders.")
    ...
    for planet_name, ephem in ephems.items():
        ...
        folder_path = os.path.join('D:/nrebig/tess/lightcurves', folder_name)
        fits_files = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith('.fits')]
        if not fits_files:
            continue

        # Load the first available FITS file
        ...
        try:
            ...
            # TESS time is BTJD (BJD - 2457000)
            t0 = ephem['t0']
            if t0 > 2450000:
                t0_btjd = t0 - 2457000.0
            else:
                t0_btjd = t0

            mask_in, mask_out = create_ephemeris_mask(time, t0_btjd, period, dur_days, buffer_factor=2.0)
            ...
            res_raw = calculate_log_snras(sub_f, sub_m, time=sub_t, period=period, t0=t0_btjd, apply_shape_correction=False)
            # Compute WITH shape correction
            res_corr = calculate_log_snras(sub_f, sub_m, time=sub_t, period=period, t0=t0_btjd, apply_shape_correction=True)
    ...
    if results:
        ...
        print("\n" + "=" * 75)
        print("SUMMARY TABLE OF TESTED TARGETS:")
        print("=" * 75)
